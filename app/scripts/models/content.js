...
define([
    ...
    var getDocBegin = _.memoize(function (title) {
        return _.findIndex(allLines, function (line) {
    ...
    var getDocEnd = _.memoize(function (docBegin) {
        var lines = _.rest(allLines, docBegin);
    ...
    // Remove markers, `//{` in the first line and `//}` in the last
    // line of the code block.
    function removeMarkers(codeLines) {
        var length = codeLines.length;

        if (codeLines[0] === '//{' && codeLines[length - 1] === '//}') {
            codeLines.shift();
            codeLines.pop();
        }
        return codeLines;
    ...
    function extractContent(title) {
        ...
        return {
            docLines: allLines.slice(docBegin, docEnd),
            codeLines: allLines.slice(codeBegin, codeEnd)
        };
    }
    ...
    var ContentModel = Backbone.Model.extend({
        ...
        update: function (ch, sec) {
            var title = common.getTitle(ch, sec),
                ...
                codeLines = removeMarkers(dedent(content.codeLines)),
                code = _.escape(codeLines.join('\n'));
